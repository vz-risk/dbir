...
function addTooltip(circle) {
    ...
    if (split.length == 1) text = split[split.length - 1];else text = split[split.length - 1];
    var tooltip = d3.select("#plot").append("text").style('font-size', '.5em').text(text.toUpperCase()).attr("transform", "translate(" + x + "," + y + ")rotate(-45)").attr("class", function () {
...
function drawNodes(nodes) {
    ...
    }).attr('opacity', 0.4).on('mouseover', function (d, i) {
        var thisData = d;
        if (d.selected) {
            return;
        }
    ...
    node.select('.nodeOutline').attr("cx", function (d, i) {
        ...
        if (!d.active) return 0;
        if (d.name == "start" || d.name == "end") return radius;
        return radius + .1;
    });
    ...
    }).attr('class', 'nodeCount').style('pointer-events', 'none').style('text-anchor', 'middle').style('opacity', 0).attr('dy', '1.35em');
